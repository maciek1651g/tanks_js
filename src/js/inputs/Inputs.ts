...
export default class Inputs {
    ...
    public get left() {
        return (
            this.keys.left.isDown || this.keys.A.isDown || this.padAxisH === -1
        );
    }

    public get right() {
        return (
            this.keys.right.isDown || this.keys.D.isDown || this.padAxisH === 1
        );
    }

    public get up() {
        return (
            this.keys.up.isDown || this.keys.W.isDown || this.padAxisV === -1
        );
    }

    public get down() {
        return (
            this.keys.down.isDown || this.keys.S.isDown || this.padAxisV === 1
        );
    }
